...
export default function PathPage() {
  ...
  const currentRealm =
    PATH_NODES.find(n => n.status === 'active')?.realm ?? 'Foundation';
  ...
  return (
    ...
    >
      <button
        ...
        className={clsx(
          'w-full p-5 rounded-lg border-2 text-left transition',
          node.status === 'completed' && 'border-green-600 bg-green-900/20',
          node.status === 'active' && 'border-imperial-gold bg-imperial-gold/10',
          node.status === 'locked' && 'opacity-50 cursor-not-allowed'
        )}
      >
        <h3 className="text-xl font-headline text-imperial-gold">
          {node.title}
        </h3>
...
function QuestDetails({
  ...
  return (
    <div className="space-y-4">
      {node.quests.map(quest => {
        const done = quest.completed || completedQuests.has(quest.id);
          ...
          >
            ...
            {!done && node.status === 'active' && (
              <Button
                size="sm"
                variant="primary"
                className="bg-imperial-gold text-black hover:bg-imperial-gold/90"
                onClick={() => onCompleteQuest(quest.id)}
              >
                Complete
              </Button>
